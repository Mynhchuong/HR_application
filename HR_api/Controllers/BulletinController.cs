...
public class BulletinController : ControllerBase
{
    ...
    public async Task<IActionResult> Save([FromBody] SaveBulletinRequest model)
    {
        try
        {
            var (success, message, id) = await _bulletinService.SaveAsync(model);
    ...
    public async Task<IActionResult> Publish(int id, string? loginUser)
    {
        try
        {
            var (success, message, fcmSent) = await _bulletinService.PublishAsync(id, loginUser);
            return Ok(new { success, message, fcmSent });
        }
        catch (Exception ex)
        {
            return Ok(new { success = false, message = ex.Message });
    ...
    public async Task<IActionResult> Unpublish(int id, string? loginUser)
    {
        try
        {
            var (success, message) = await _bulletinService.UnpublishAsync(id, loginUser);
    ...
    public async Task<IActionResult> TogglePin(int id, string? loginUser)
    {
        try
        {
            ...
            if (newPin == 1)
            {
                int currentPinned = await _bulletinService.CountPinnedAsync(excludeId: id);
