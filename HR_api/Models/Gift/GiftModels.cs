...
// "Đợt quà" — vd Quà sinh nhật tháng 9/2026
public class GiftBatchItem
{
    ...
    public string?  CATEGORY_NAME     { get; set; }
    public string   RECEIVE_FROM_DATE { get; set; } = ""; // yyyy-MM-dd
...
public class GiftBatchCreateRequest
{
    ...
    public string? RECEIVE_TO_DATE   { get; set; }
    public string? REMARK            { get; set; }
    // Toàn công ty -> tự động áp dụng cho mọi NV đang làm việc (ECM100, chưa nghỉ), HR khỏi
    // import Excel; LOCATION dùng chung cho tất cả (thay vì import riêng từng dòng).
    public bool    IS_COMPANY_WIDE   { get; set; }
    public string? LOCATION          { get; set; }
    public string  ACTOR_EMPCD       { get; set; } = "";
}
...
// 1 dòng danh sách người nhận — dùng chung cho admin list / report / export / worker "quà của tôi".
public class GiftRecipientListItem
{
    ...
    public string   CONFIRM_STATUS     { get; set; } = "NONE"; // NONE | PENDING_CONFIRM | CONFIRMED
    public string?  CONFIRMED_DT       { get; set; }

    public string   STATUS             { get; set; } = "READY"; // READY|DELIVERED|COMPLETED|EXPIRED (tính live)
