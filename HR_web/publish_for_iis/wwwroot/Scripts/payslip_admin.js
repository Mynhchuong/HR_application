...
$(document).ready(function () {
    ...
    $('#btnSaveConfig').on('click', function () {
        const items = [];
        $('#tbodyConfigItems tr').each(function () {
            const $row = $(this);
            const id = $row.find('.item-visibility-toggle').data('id');
            const isVisible = $row.find('.item-visibility-toggle').is(':checked') ? 1 : 0;
            const itemName = $row.find('.item-name-input').val().trim();
